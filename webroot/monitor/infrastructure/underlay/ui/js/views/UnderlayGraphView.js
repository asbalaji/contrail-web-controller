        ...
        render: function() {
            ...
            this.listenTo(graphModel, "change", function(updatedGraphModel) {
                if (contrail.checkIfExist(updatedGraphModel.elementsDataObj)) {
                    self.addElementsToGraph(updatedGraphModel.elementsDataObj.networkElements,
                        updatedGraphModel);
                }
            });
            ...
            this.network.on("blurNode", function(node){
                ...
                window.tooltipConfigWidth = 0;
                window.tooltipConfigHeight = 0;
            });
            this.network.on("showPopup", function(elementId){
                ...
                window.tooltipConfigWidth = tooltipConfig.dimension.width;
                window.tooltipConfigHeight = tooltipConfig.dimension.height;
                $('.popover').remove();
                var tt = $(".vis-network-tooltip").popover({
                    ...
                    placement: function (context, src) {  //src is mouse position
                        var srcOffset = $(src).offset(),
                            ...
                            tooltipWidth = window.tooltipConfigWidth;
                        ...
                        if (srcOffset.left-5 > tooltipWidth) {
                            return 'left';
                        } else if (bodyWidth - (srcOffset.left-5) - srcWidth > tooltipWidth){
                            return 'right';
                ...
                remote: {
                    ...
                    successCallback: function(response, underlayGraphModel) {
                        ...
                        if (contrail.checkIfExist(underlayGraphModel.elementsDataObj)) {
                            var elements = underlayGraphModel.elementsDataObj.networkElements;
                            if (elements.length > 0) {
                                _this.addElementsToGraph(underlayGraphModel.elementsDataObj.networkElements,
                                underlayGraphModel);
